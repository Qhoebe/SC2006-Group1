          ...
          if (favourites) {
            fetch(`/favourites/findAll`, {
              ...
              .then(favData => {
                ...
                const filteredFacilityData = data.filter(facility => placeIDs.includes(facility.CarParkID));
          ...
          if (favourites) {
            fetch(`/favourites/findAll`, {
              ...
              .then(favData => {
                ...
                const placeIDs = favData.map(fav => fav.placeID);
                // Filter facilityData such that only the objects whose CarParkID is within all the placeID
                const filteredFacilityData = (data.results).filter(facility => placeIDs.includes(facility.PlaceID));
                // Do something with filteredFacilityData
                setFacilityData(filteredFacilityData);
              })
          } else {
        ...
        >
          ...
          <div className="overlayContainer">
            ...
            <div>
              ...
              >Fuel Station</button>
              <button className="overlayButton" onClick={() => setFavourites(!favourites)}>
                <FontAwesomeIcon icon={faBookmark} color={favourites ? 'red' : 'black'} size='lg' />
              </button>
            </div>
